...
def get_contract_prompt_template() -> str:
    # 该提示用于指导合同输出为正式范本，避免 PRD/Markdown/引用内容
    return (
        "你是资深法务专家。\n"
        "请根据以下需求文档（PRD）和参考资料（RAG），起草一份正式可签署的商业合同范本。\n"
        "要求：\n"
        "1. 只输出合同正文，不要输出 Markdown 标记、代码块围栏、PRD 标题结构。\n"
        "2. 不要输出 <think> 或任何思考过程内容。\n"
        "3. 不要包含“来源依据/参考资料”等引用段落。\n"
        "4. 合同内容包含：合同标题、甲乙双方、合作背景、服务内容、报价条款（含结构化报价清单）、交付标准、验收方式、知识产权、保密条款、违约责任、争议解决、签署信息。\n"
        "5. 用语正式、可直接用于签署。\n"
        "\n"
        "PRD 内容：\n"
        "{{prd}}\n"
